...
export const resize = async (inputFileName: string, outputFilename: string, options: Options) => {
    const inputDir = path.join(__dirname, "..", "/bin/in", inputFileName); 
    const tempDir = path.join(__dirname, "..", "/tmp", "/temp__img.png")
    ...
    if (!options.rgba)    {
        await command(`convert ${tempDir} -colorspace gray -compress none -depth 8 ${outputDir}`);
    } else {
